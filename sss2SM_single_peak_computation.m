function makeplot = sss2SM_single_peak_computation(data,paramstruct) 
...
if nargin > 1 ;   %  then paramstruct has been added
  ...
  if isfield(paramstruct,'promstart0') ;    %  then change to input value
    promstart0 = getfield(paramstruct,'promstart0') ; 
  end ;

  if isfield(paramstruct,'promend0') ;    %  then change to input value
    promend0 = getfield(paramstruct,'promend0') ; 
  end ;
  
  if isfield(paramstruct,'centercut') ;    %  then change to input value
    centercut = getfield(paramstruct,'centercut') ; 
  end ;
  
  if isfield(paramstruct,'ensg') ;    %  then change to input value
    ensg = getfield(paramstruct,'ensg') ; 
  end ;
  if isfield(paramstruct,'chr_id') ;    %  then change to input value
    chr_id = getfield(paramstruct,'chr_id') ; 
  end ;
  ...
  if stype ~= 0 ;   %  then want to add some SSS parts
    ...
    if circleflag == 1 ;    %  then want to draw small ESS circles
      ...
      elseif igrid == 2 ;    %  then are working with 2x2 blocks
        mo2 = floor(m/2) ;
        no2 = floor(n/2) ;
        smallessflagdec = conv2(double(smallessflag),ones(2,2),'valid') ;
          %  do 2x2 simple sum
    ...
    if allsmalless ;          %  then Effective Sample Size is nowhere
      ...
      if stype == 2  |  stype == 3 ;  
        ...
        sss2dotpSM_peak_computation(mdot,vsigc,vsigg,colormask,mcol,n,m,stype,...
                                                     igrid,chr_id,promstart0,promend0,length,h,centercut) ;
      end ;
